                                ...
                                <table class="table table-striped table-bordered table-hover">
                                    ...
                                    @foreach($data as $item)
                                        <tr>
                                            ...
                                            <td class="text-center v-align-m">
                                                @if ($item->is_confirmed == 1 && $item->is_cancelled == 0)
                                                    Подтверждена
                                                @elseif($item->is_confirmed == 0 && $item->is_cancelled == 0)
                                                    <a href="Javascript:void(0);" data-request-id="{{ $item->id }}" onclick="confirmRequisition(this)">Подтвердить</a>
                                                    /
                                                    <a href="Javascript:void(0);" data-request-id="{{ $item->id }}" onclick="confirmNoRequisition(this)">Отклонить</a>
                                                @elseif($item->is_confirmed == 2 && $item->is_cancelled == 1)
                                                    Отклонена
                                                @endif
                                            </td>
                                        </tr>
...
@section('extra-script')
    <script>
        ...
        function confirmNoRequisition(el) {
            let btn = $(el),
                id = btn.attr('data-request-id');
            btn.removeAttr('href');
            btn.removeAttr('onclick');
            if (confirm('Отклонить?')) {
                axios.post('/admin/requisition/'+id, { is_confirmed: 2, is_cancelled: 1})
                    .then((response) => {
                        location.reload();
                    })
                    .catch((error) => { console.error(error); })
