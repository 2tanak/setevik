    ...
    <div class="events-detail-wrapper">
        ...
        <div class="tabs-left">
                ...
                <div class="tab-pane active" id="tab-r1">
                    <fieldset>
                        <div class="form-horizontal">
                            ...
                            <div class="form-group">
                                <label class="col-md-2 control-label">Дата проведения:</label>
                                <div class="col-md-6">
                                    <input v-model="date" class="form-control">
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="col-md-2 control-label">Формат проведения:</label>
                                <div class="col-md-6">
                                    <input v-model="format" class="form-control">
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="col-md-2 control-label">Место проведения:</label>
                                <div class="col-md-6">
            ...
            <div class="text-center">
                ...
                <a href="/admin/sib/events" class="btn btn-default">Отменить</a>
            </div>
        </div>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            init: function () {
                ...
                th.date         = th.item.date || '';
                th.format       = th.item.format || '';
                th.location     = th.item.location || '';
            ...
            save: function (params, callback) {
                ...
                if (th.id) {
                    ...
                    formData.append('price', th.price);
                    formData.append('date', th.date);
                    formData.append('format', th.format);
                    formData.append('location', th.location);
                    ...
                    axios.post('/admin/sib/events', formData, {headers: {'Content-Type': 'multipart/form-data'}})
                        .then((response) => {
                            th.loading      = false;
                            ...
                            th.price        = response.data.price;
                            th.date         = response.data.date;
                            th.format       = response.data.format;
                            th.location     = response.data.location;

                            console.debug('save:response', response.data);
                            console.debug('save:callback', callback);

                            if (callback) {
                                callback();
                            } else {
                                location.href = '/admin/sib/events';
                            }
            ...
            update: function (callback) {
                ...
                axios.post('/admin/sib/events/'+th.id, formData, {headers: {'Content-Type': 'multipart/form-data'}})
                    .then((response) => {
                        th.loading      = false;
                        th.title        = response.data.title;
                        th.announce     = response.data.announce;
            ...
            _removeFile: function (index) {
                ...
                if (confirm('Удалить?')) {
                    ...
                    axios.delete('/admin/sib/events/'+th.id+'/files/'+th.list[index].id)
                        .then((response) => {
            ...
            _uploadFile: function () {
                ...
                if (file.files.length > 0) {
                    ...
                    axios.post('/admin/sib/events/'+th.id+'/files', formData, {headers: {'Content-Type': 'multipart/form-data'}})
                        .then((response) => {
                            th.loading = false;
